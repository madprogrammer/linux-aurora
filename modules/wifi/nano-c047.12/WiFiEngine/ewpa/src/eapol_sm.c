/*
 * WPA/WAPI Supplicant / EAPOL state machines
...
struct eapol_sm {
	...
	SUPP_PAE_state_TYPE SUPP_PAE_state; /* dot1xSuppPaeState */
	/* Variables */
	Boolean userLogoff;
	Boolean logoffSent;
	...
	KEY_RX_state_TYPE KEY_RX_state;
	/* Variables */
	Boolean rxKey;

	/* Supplicant Backend state machine */
	SUPP_BE_state_TYPE SUPP_BE_state; /* dot1xSuppBackendPaeState */
	/* Variables */
	Boolean eapNoResp;
	Boolean eapReq;
	Boolean eapResp;
...
#ifdef NR_WPA_EAPOL_TIMER_RESTART
	if (sm->authWhile == 0 &&
	    sm->heldWhile == 0 &&
	    sm->startWhen == 0 &&
	    sm->idleWhile == 0)
		wpa_printf(MSG_DEBUG, "not restarting EAPOL port timer");
	else
#endif
...
#ifdef NR_WPA_EAPOL_TIMER_RESTART
static inline void
start_eapol_timer(struct eapol_sm *sm)
{
	wpa_printf(MSG_DEBUG, "starting EAPOL port timer");
...
/**
 * eapol_sm_rx_wapi - Process received WAPI frames
 * @sm: Pointer to WAPI state machine allocated with eapol_sm_init()
 * @src: Source MAC address of the WAPI packet
 * @buf: Pointer to the beginning of the WAPI data (WAPI header)
 * @len: Length of the WAPI frame
 * Returns: 1 = WAPI frame processed, 0 = not for EAPOL state machine,
 * -1 failure
 */
int eapol_sm_rx_wapi(struct eapol_sm *sm, const u8 *src, const u8 *buf,
		     size_t len)
{
  /* UNIMPLEMENTED */
  /*wpa_printf(MSG_DEBUG, "eapol_sm_rx_wapi: UNIMPLEMENTED!");*/
  return 0;
}
